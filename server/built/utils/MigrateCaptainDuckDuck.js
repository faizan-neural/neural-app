...
class MigrateCaptainDuckDuck {
    constructor(dataStore, authenticator) {
        ...
        this.oldFilePath = path.join(CaptainConstants_1.default.captainDataDirectory, 'config.conf');
    }
    migrateIfNeeded() {
        // Captain Boots-up
        ...
        if (!fs.pathExistsSync(oldFilePath)) {
            Logger_1.default.d('Migration not needed, skipping.');
    ...
    performMigration() {
        Logger_1.default.d('Migration starting now...');
        const self = this;
        const oldFilePath = self.oldFilePath;
        const dataStore = this.dataStore;
        return Promise.resolve()
            .then(function (data) {
            return fs.readJson(path.join(CaptainConstants_1.default.captainDataDirectory, 'config.conf'));
        })
            .then(function (dataRetrieved) {
            self.oldData = dataRetrieved;
        })
            .then(function () {
            Logger_1.default.d('Migrating basic configs...');
            ...
            const authObj = JSON.parse(fs
                .readFileSync(`/run/secrets/captain-reg-auth${Number(oldData.captainRegistryAuthSecretVer)}`)
                .toString());
            const registriesDataStore = dataStore.getRegistriesDataStore();
            if (authObj.serveraddress.endsWith(`${oldData.customDomain}:${CaptainConstants_1.default.configs.registrySubDomainPort}`)) {
                // local
            ...
            Object.keys(oldAppDefinitions).forEach((appName) => {
                ...
                const p = Promise.resolve() //
                    ...
                    return Promise.resolve()
                        ...
                        return appStore.addCustomDomainForAppForMigration(appName, hasDefaultSubDomainSsl, customDomains);
                    })
                        .then(function () {
                        ...
                        if (!deployedVersionFound) {
                            Logger_1.default.d('********* WARNING!! *********');
                            Logger_1.default.d(`** Not able to find the deployed image for ${appName}. This app might misbehave!! **`);
                        }
                        ...
                        .then(function () {
                        return appStore.updateAppDefinitionInDb(appName, '', Number(app.instanceCount), CaptainConstants_1.default.defaultCaptainDefinitionPath, app.envVars || [], app.volumes || [], [], app.nodeId || '', !!app.notExposeAsWebApp, 80, undefined, !!app.forceSsl, app.ports || [], repoInfo, self.authenticator, app.customNginxConfig, '', app.preDeployFunction, '', false, {
                            enabled: false,
                        });
                    });
                });
            ...
            .then(function () {
            Logger_1.default.d('Old data migrated to new format, deleting the old format...');
            fs.remove(oldFilePath);
        })
            .then(function () {
            Logger_1.default.d('Migration successfully done!');
            return Promise.resolve();
        });
    }
}
